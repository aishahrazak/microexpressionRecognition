...
try:
    ...
    BASE_MODEL_PATH = '{}/3class-dropout50reg-aug.h5'.format(weights)
    inputs = Input(shape=(224,224,3))
    data_augmentation = Sequential([RandomFlip('horizontal'), RandomContrast(0.2)])
    x = data_augmentation(inputs)
    x = effB0_model(x, training=False) #run in inference mode
    ...
    #train
        ...
        with open(outpFile, 'a') as f:
            f.write('Subject {}: Result counts: {}'.format(subject, cpc))
    ...
    print(e)
    util.sendNotification('3 class Aug Model Error')
else:
    util.sendNotification('3 class Aug Model completes running')